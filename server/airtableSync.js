  ...
  return (db.users || []).map((u) => {
    ...
    return {
      fields: {
        ...
        "3DA": num(u.avg),
        Role: roleLabel(u),
        Regionals: regionals.join(" · ") || (u.regionalChoice === "international" || u.regionalChoice === "world"
          ? "TSH International"
  ...
  return (db.fixtures || []).map((f) => {
    ...
    return {
      fields: {
        ...
        Division: leagueTitle(db, league),
        Week: num(f.week) || 0,
        Home: f.bye && !f.homeId ? "Bye" : home?.nickname || home?.name || "",
        Away: f.bye && !f.awayId ? "Bye" : away?.nickname || away?.name || "",
