...
- There is no lint or automated test tooling configured in this repo (no ESLint,
  Prettier, or test runner). "Build" is a no-op for the served app; validate
  changes by running the server and exercising the UI / `/api/*` endpoints.
